...
BUILD=`rbuilder build-create zimbra "$TROVE" vmware_esx_image --wait --option 'vmMemory 512' --option 'freespace 2500' --option "baseFileName zcs-${RELEASETAG}-esx"  | awk -F= '{print $NF}'`
if [ $? -eq 0 ]; then
  echo "Getting URL for Build $BUILD"
