...
BUILD=`rbuilder build-create zcs-${RELEASETAG} "$TROVE" installable_iso --wait | awk -F= '{print $NF}'`
if [ $? -eq 0 ]; then
  ISO=`rbuilder build-url $BUILD | head -1`
  wget -o $BUILDROOT/i386/zcs-${RELEASETAG}.iso $ISO
fi
BUILD=`rbuilder build-create zcs-${RELEASETAG} "$TROVE" vmware_image --wait | awk -F= '{print $NF}'`
if [ $? -eq 0 ]; then
  ISO=`rbuilder build-url $BUILD | head -1`
