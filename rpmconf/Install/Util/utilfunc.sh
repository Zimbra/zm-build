...
getPlatformVars() {
  ...
  if [ $? = 0 ]; then
    ...
    if [ $PLATFORM = "UBUNTU10_64" ]; then
      PREREQ_PACKAGES="netcat-openbsd bc sudo libidn11 libpcre3 libgmp3c2 libexpat1 libstdc++6 libperl5.10 libaio1"
      PRESUG_PACKAGES="pax perl-5.10.1 sysstat sqlite3"
    fi
    if [ $PLATFORM = "UBUNTU12_64" ]; then
      PREREQ_PACKAGES="netcat-openbsd bc sudo libidn11 libpcre3 libgmp3c2 libexpat1 libstdc++6 libperl5.14 libaio1"
      PRESUG_PACKAGES="pax perl-5.14.2 sysstat sqlite3"
    fi
    if [ $PLATFORM = "UBUNTU14_64" ]; then
      PREREQ_PACKAGES="netcat-openbsd bc sudo libidn11 libpcre3 libgmp10 libexpat1 libstdc++6 libperl5.18 libaio1 unzip"
    ...
    if [ $PLATFORM = "RHEL6_64" -o $PLATFORM = "CentOS6_64" ]; then
      ...
      PREREQ_PACKAGES="nc bc sudo libidn gmp libaio"
