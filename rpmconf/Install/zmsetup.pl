...
my @packageList = (
    "zimbra-core",      "zimbra-ldap",
    "zimbra-logger",    "zimbra-mta",
    "zimbra-dnscache",  "zimbra-snmp",
    "zimbra-store",     "zimbra-apache",
    "zimbra-spell",     "zimbra-convertd",
    "zimbra-memcached", "zimbra-proxy",
    "zimbra-archiving", "zimbra-imapd",
);
...
my @webappList = ( "service", "zimbra", "zimbraAdmin", "zimlet", );
...
my $prevVersion = "";
our $curVersion = "";
my ( $prevVersionMinor, $prevVersionMajor, $prevVersionMicro,
    $prevVersionBuild );
my (
    $curVersionMinor,      $curVersionMajor, $curVersionMicro,
    $curVersionMicroMicro, $curVersionType,  $curVersionBuild
);
our $newinstall = 1;
chomp(
...
if ( !$newinstall ) {
    ...
    if ( -f "/opt/zimbra/conf/ca/ca.pem" ) {
        ...
        main::runAsZimbra(
            "/opt/zimbra/bin/zmcertmgr addcacert /opt/zimbra/conf/ca/ca.pem");
    }
...
setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionMajor',
    $curVersionMajor );
setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionMinor',
    $curVersionMinor );
setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionMicro',
    $curVersionMicroMicro );
setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionType',
    $curVersionType );
setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionBuild',
    $curVersionBuild );
...
sub defineInstallWebapps {
    if ( !defined $config{INSTALL_WEBAPPS} ) {
        ...
        if ( $config{UIWEBAPPS} eq "yes" ) {
            $config{INSTALL_WEBAPPS} =
              "$config{INSTALL_WEBAPPS} zimbra zimbraAdmin";
        }
    }
}
...
sub getInstalledWebapps {
    ...
    foreach my $app (@webappList) {
            ...
            || ( !$newinstall && isServiceEnabled($app) ) )
        {
            ...
            if ( $newinstall
                || ( !$newinstall && $installedWebapps{$app} ne "Enabled" ) )
            {
...
sub getSystemStatus {
    ...
    if ( isEnabled("zimbra-ldap") ) {
        if ( -f "/opt/zimbra/data/ldap/mdb/db/data.mdb" ) {
            $ldapConfigured = 1;
            $ldapRunning =
              0xffff & system("/opt/zimbra/bin/ldap status > /dev/null 2>&1");
            if ($ldapRunning) {
                $ldapRunning = 0;
            }
            else {
    ...
    if ( isEnabled("zimbra-store") ) {
        if ( -d "/opt/zimbra/db/data/zimbra" ) {
            $sqlConfigured = 1;
            $sqlRunning    = 0xffff &
              system("/opt/zimbra/bin/mysqladmin status > /dev/null 2>&1");
...
sub setLdapDefaults {
    ...
    if ( $serverid ne "" ) {
        ...
        $config{zimbraDNSTCPUpstream} =
          getLdapServerValue("zimbraDNSTCPUpstream");
    ...
    if ( isNetwork() && isEnabled("zimbra-store") ) {
        $config{zimbraBackupReportEmailRecipients} =
          getLdapConfigValue("zimbraBackupReportEmailRecipients");
        ...
        $config{zimbraBackupReportEmailSender} =
          getLdapConfigValue("zimbraBackupReportEmailSender");
    ...
    # force confirmation of choice during upgrade if this was never setup before
        if (  !$newinstall
            && $config{zimbraVersionCheckNotificationEmail} eq ""
            && !$options{c} )
        {
            $config{VERSIONUPDATECHECKS} = "";
        }
    ...
    $config{EphemeralBackendURL} =
      getLdapConfigValue("zimbraEphemeralBackendURL");
    $config{USEEPHEMERALSTORE} = "yes"
      if ( $config{EphemeralBackendURL} ne "" );
    ...
    $config{zimbraFeatureTasksEnabled} =
      getLdapCOSValue("zimbraFeatureTasksEnabled");
    ...
    $config{zimbraFeatureBriefcasesEnabled} =
      getLdapCOSValue("zimbraFeatureBriefcasesEnabled");
...
sub setDefaults {
    ...
    if ( -f "/opt/zimbra/common/jetty_home/start.jar" ) {
        ...
        $config{mailboxd_keystore} =
          "$config{mailboxd_directory}/conf/keystore";
...
sub getInstallStatus {
    ...
    (
        $prevVersionMajor, $prevVersionMinor,
        $prevVersionMicro, $prevVersionBuild
    ) = $prevVersion =~ /(\d+)\.(\d+)\.(\d+_[^_]*)_(\d+)/;
    ( $curVersionMajor, $curVersionMinor, $curVersionMicro, $curVersionBuild )
      = $curVersion =~ /(\d+)\.(\d+)\.(\d+_[^_]*)_(\d+)/;
    ( $curVersionMicroMicro, $curVersionType ) =
      $curVersionMicro =~ /(\d+)_(.*)/;

    if ( $options{d} ) {
        progress "done.\n";
        progress
"Previous version  maj:$prevVersionMajor minor:$prevVersionMinor micro:$prevVersionMicro build:$prevVersionBuild\n";
        progress
"Current version  maj:$curVersionMajor minor:$curVersionMinor micro:$curVersionMicro build:$curVersionBuild\n";
    }
}
    ...
    if ( isEnabled("zimbra-ldap") ) {
        if ( isLdapMaster() ) {
            $config{ldap_bes_searcher_password} =
              getLocalConfig("ldap_bes_searcher_password");
...
sub setEphemeralBackendURL {
    my $rc = 1;
    while ( $rc != 0 ) {
        my $newURL = ask( "Value for zimbraEphemeralBackendURL:",
            $config{EphemeralBackendURL} );
        $rc = runAsZimbra(
            "zmjava com.zimbra.cs.ephemeral.EphemeralStore -u $newURL");
        if ( $rc == 0 ) {
        ...
        if ( askYN( "Revert to storing ephemeral attributes in Ldap?", "No" ) eq
            "yes" )
        {
            $config{USEEPHEMERALSTORE} = "no";
            delete( $config{EphemeralBackendURL} )
...
"\n\nDNS ERROR - none of the MX records for $config{CREATEDOMAIN}\n"
    ...
    my ( $vcFromUser, $vcFromDomain ) =
      split( '@', $config{zimbraVersionCheckNotificationEmailFrom} );
    $config{zimbraVersionCheckNotificationEmailFrom} =
      $vcFromUser . '@' . $config{CREATEDOMAIN}
      if ( $vcFromDomain eq $oldDomain );

    my ( $vcUser, $vcDomain ) =
      split( '@', $config{zimbraVersionCheckNotificationEmail} );
    $config{zimbraVersionCheckNotificationEmail} =
      $vcUser . '@' . $config{CREATEDOMAIN}
...
sub setVersionCheckNotificationEmail {
    while (1) {
        my $new = ask(
            "Version update destination address:",
            $config{zimbraVersionCheckNotificationEmail}
        );
...
sub setMasterDNSIP {
    while (1) {
        my $new =
          ask( "IP Address(es) of Master DNS Server(s), space separated:",
            $config{zimbraDNSMasterIP} );
        my @IPs = split( ' ', $new );
...
"Admin user must be a valid email account [$u\@$config{CREATEDOMAIN}]\n"
       ...
       # this prevents us from trying to provision in a non-existent domain
        ...
        $config{zimbraBackupReportEmailRecipients} = $new
          if (
            $config{zimbraBackupReportEmailRecipients} eq $config{CREATEADMIN}
          );
...
sub setLdapBesSearchPass {
    while (1) {
        my $new = askPassword( "Password for ldap BES user (min 6 characters):",
            $config{ldap_bes_searcher_password} );
        if ( length($new) >= 6 ) {
    ...
    if ( $config{CREATEADMIN} ne "" ) {
        while (1) {
            if ( $config{CREATEADMINPASS} eq "" ) {
                $config{CREATEADMINPASS} = genRandomPass();
            }
            my $new =
              askPassword(
                "Password for $config{CREATEADMIN} (min 6 characters):",
...
sub setSnmpTrapHost {
    $config{SNMPTRAPHOST} =
      askNonBlank( "SNMP Trap host:", $config{SNMPTRAPHOST} );
}
...
sub setProxyMode {
    while (1) {
        my $m = askNonBlank(
"Please enter the proxy server mode (http,https,both,mixed,redirect)",
            $config{PROXYMODE}
        );
        if ( $config{zimbra_require_interprocess_security} ) {
...
sub setHostName {
    ...
    if ( $config{CREATEDOMAIN} eq $old ) {
        ...
        my ( $u, $d ) =
          split( '@', $config{zimbraBackupReportEmailRecipients} );
        $config{zimbraBackupReportEmailRecipients} =
...
sub setHttpPort {
    $config{HTTPPORT} =
      askNum( "Please enter the HTTP server port:", $config{HTTPPORT} );
...
sub setHttpsPort {
    $config{HTTPSPORT} =
      askNum( "Please enter the HTTPS server port:", $config{HTTPSPORT} );
...
sub setImapPort {
    $config{IMAPPORT} =
      askNum( "Please enter the IMAP server port:", $config{IMAPPORT} );
...
sub setImapSSLPort {
    $config{IMAPSSLPORT} =
      askNum( "Please enter the IMAP SSL server port:", $config{IMAPSSLPORT} );
...
sub setPopPort {
    $config{POPPORT} =
      askNum( "Please enter the POP server port:", $config{POPPORT} );
...
sub setPopSSLPort {
    $config{POPSSLPORT} =
      askNum( "Please enter the POP SSL server port:", $config{POPSSLPORT} );
...
sub setSpellUrl {
    $config{SPELLURL} =
      askNonBlank( "Please enter the spell server URL:", $config{SPELLURL} );
}

sub setLicenseFile {
    $config{LICENSEFILE} =
      askFileName( "Enter the name of the file that contains the license:",
        $config{LICENSEFILE} );
...
sub setIPMode {
    while (1) {
        my $new = askPassword( "IP Mode for Zimbra (ipv4, both, ipv6):",
            $config{zimbraIPMode} );
...
sub setEnabledDependencies {
    ...
    if ( isEnabled("zimbra-mta") ) {
        if ($newinstall) {
            ...
            $config{RUNARCHIVING} =
              ( isServiceEnabled("archiving") ? "yes" : "no" );
            $config{RUNCBPOLICYD} =
    ...
    if ( isEnabled("zimbra-core") ) {
        if ($newinstall) {
            ...
            if ( isNetwork() ) {
                $config{RUNVMHA} =
                  ( isServiceEnabled("vmware-ha") ? "yes" : "no" );
            }
            else {
                $config{RUNVMHA} = "no";
            }
...
sub createCommonMenu {
    ...
    $config{USEEPHEMERALSTORE} = "no"
      unless ( exists $config{USEEPHEMERALSTORE} );
    $$lm{menuitems}{$i} = {
            ...
            || $config{LDAPREPLICATIONTYPE} ne "replica" )
        {
            if ( $config{ldap_bes_searcher_password} eq "" ) {
                ...
                $config{LDAPBESSEARCHSET} = "set"
                  unless ( $config{LDAPBESSEARCHSET} eq "Not Verified" );
            }
            $$lm{menuitems}{$i} = {
...
sub createMtaMenu {
    ...
    if ( isEnabled($package) ) {
        ...
        if (   isEnabled("zimbra-archiving")
            || isComponentAvailable("archiving") )
        {
            $$lm{menuitems}{$i} = {
...
"Couldn't bind to $config{LDAPHOST} as $config{zimbra_ldap_userdn}\n"
    ...
    if ( $config{LDAPHOST} eq $config{HOSTNAME} ) {
        ...
        my $binduser =
          "uid=zmbes-searcher,cn=appaccts,$config{ldap_dit_base_dn_config}";
...
sub configCASetup {
    ...
    if ( !$newinstall ) {
        if ( -f "/opt/zimbra/conf/ca/ca.pem" ) {
            my $rc = runAsRoot(
"/opt/zimbra/common/bin/openssl verify -purpose sslserver -CAfile /opt/zimbra/conf/ca/ca.pem /opt/zimbra/conf/ca/ca.pem | egrep \"^error 10\""
            );
...
# either the ca is expired from the test above or the ca directory doesn't exist.
    ...
    if ( isLdapMaster() ) {
        $needNewCA = "-new"
          if ( !-d "/opt/zimbra/ssl/zimbra/ca" || $needNewCert eq "-new" );
    }
...
sub configSetupLdap {
        ...
        && ( $config{LDAPHOST} eq $config{HOSTNAME} ) )
    {
        ...
        if (
            my $rc = runAsZimbra(
"/opt/zimbra/libexec/zmldapinit \'$config{LDAPROOTPASS}\' \'$config{LDAPADMINPASS}\'"
            )
          )
        {
            progress("failed. ($rc)\n");
            ...
            if ( $ldapRepChanged == 1 ) {
                ...
                runAsZimbra(
                    "/opt/zimbra/bin/zmldappasswd -l \'$config{LDAPREPPASS}\'");
                progress("done.\n");
            }
            if ( $ldapPostChanged == 1 ) {
                progress("Setting Postfix password...");
                runAsZimbra(
                    "/opt/zimbra/bin/zmldappasswd -p \'$config{LDAPPOSTPASS}\'"
                );
                progress("done.\n");
            }
            if ( $ldapAmavisChanged == 1 ) {
                ...
                runAsZimbra(
"/opt/zimbra/bin/zmldappasswd -a \'$config{LDAPAMAVISPASS}\'"
                );
                progress("done.\n");
            }
            if ( $ldapNginxChanged == 1 ) {
                ...
                runAsZimbra(
"/opt/zimbra/bin/zmldappasswd -n \'$config{ldap_nginx_password}\'"
                );
                progress("done.\n");
            }
            if ( $ldapBesSearcherChanged == 1 ) {
                ...
                runAsZimbra(
"/opt/zimbra/bin/zmldappasswd -b \'$config{ldap_bes_searcher_password}\'"
                );
                progress("done.\n");
            }
        }
...
"$proto://$config{HOSTNAME}:$config{LDAPPORT} $ldapMasterUrl"
                    ...
                    $rc = runAsZimbra(
"/opt/zimbra/libexec/zmldapenable-mmr -s $config{LDAPSERVERID} -m $ldapMasterUrl"
                    );
                }
                else {
                    $rc =
...
"You will have to correct the problem and manually enable replication.\n"
        ...
        if (   $ldapRootPassChanged
            ...
            || $ldapAmavisChanged
            || $ldapNginxChanged
            || $ldapBesSearcherChanged )
        {
            if ($ldapRootPassChanged) {
                progress("Setting ldap root password...");
                runAsZimbra(
                    "/opt/zimbra/bin/zmldappasswd -r $config{LDAPROOTPASS}");
                progress("done.\n");
            }
            if ($ldapAdminPassChanged) {
                ...
                if ( $config{LDAPHOST} eq $config{HOSTNAME} ) {
                    runAsZimbra(
                        "/opt/zimbra/bin/zmldappasswd $config{LDAPADMINPASS}");
                }
                else {
...
"/opt/zimbra/bin/zmldappasswd -b $config{ldap_bes_searcher_password}"
                    ...
                    setLocalConfig( "ldap_bes_searcher_password",
                        "$config{ldap_bes_searcher_password}" );
                }
                progress("done.\n");
            }
...
sub configLDAPSchemaVersion {
    ...
    if ( isEnabled("zimbra-ldap") ) {
        progress(
            "Updating zimbraLDAPSchemaVersion to version '$ldapSchemaVersion'\n"
        );
...
sub configSetupEphemeralBackend {
    if ( exists( $config{EphemeralBackendURL} ) ) {
        setLdapGlobalConfig( "zimbraEphemeralBackendURL",
            "$config{EphemeralBackendURL}" );
    }
...
sub configCreateCert {
    ...
    if ( $configStatus{configCreateCert} eq "CONFIGURED"
        && -d "/opt/zimbra/ssl/zimbra/server" )
    {
    ...
    if ( !$newinstall ) {
        my $rc = runAsZimbra(
            "/opt/zimbra/bin/zmcertmgr verifycrt comm > /dev/null 2>&1");
        if ( $rc != 0 ) {
            $rc = runAsZimbra(
                "/opt/zimbra/bin/zmcertmgr verifycrt self > /dev/null 2>&1");
            if ( $rc != 0 ) {
...
"New self-signed certificates will be generated and installed.\n"
    ...
    if ( isInstalled("zimbra-imapd") ) {
        if (   !-f "$config{imapd_keystore}"
            && !-f "/opt/zimbra/conf/server.crt" )
        {
            progress("Creating SSL zimbra-imapd certificate...");
            $rc =
              runAsZimbra("/opt/zimbra/bin/zmcertmgr createcrt $needNewCert");
            if ( $rc != 0 ) {
                progress("failed.\n");
                exit 1;
            }
    ...
    if ( isInstalled("zimbra-store") ) {
        if (   !-f "$config{mailboxd_keystore}"
            && !-f "/opt/zimbra/ssl/zimbra/server/server.crt" )
        {
    ...
    if ( isInstalled("zimbra-ldap") ) {
        if (   !-f "/opt/zimbra/conf/slapd.crt"
            && !-f "/opt/zimbra/ssl/zimbra/server/server.crt" )
        {
    ...
    if ( isInstalled("zimbra-mta") ) {
        if (   !-f "/opt/zimbra/conf/smtpd.crt"
            && !-f "/opt/zimbra/ssl/zimbra/server/server.crt" )
        {
    ...
    if ( isInstalled("zimbra-proxy") ) {
        if (   !-f "/opt/zimbra/conf/nginx.crt"
            && !-f "/opt/zimbra/ssl/zimbra/server/server.crt" )
        {
...
sub configInstallCert {
        ...
        && $needNewCert eq "" )
    {
        ...
        if ( !( -f "$config{mailboxd_keystore}" ) || $needNewCert ne "" ) {
            ...
            detail("$needNewCert was ne \"\".")
              if ( $needNewCert ne "" );
            $rc =
              runAsZimbra("/opt/zimbra/bin/zmcertmgr deploycrt $ssl_cert_type");
            if ( $rc != 0 ) {
                progress("failed.\n");
                exit 1;
            }
    ...
    if (   $configStatus{configInstallCertImap} eq "CONFIGURED"
        && $needNewCert eq "" )
    {
...
sub configSpellServer {
    ...
    if ( $config{USESPELL} eq "yes" ) {
        ...
        my $rc =
          setLdapServerConfig( "zimbraSpellCheckURL", $config{SPELLURL} );
        progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
    }
...
sub configConvertdURL {
    ...
    if ( $tmpval eq "" ) {
        ...
        my $rc = setLdapGlobalConfig( "zimbraConvertdURL",
            "http://$host:7047/convert" );
        progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
...
# new service node to zimbraReverseProxyAvailableLookupTargets.  Otherwise do not
        ...
        if ($adding) {
            progress(
"Adding $config{HOSTNAME} to zimbraReverseProxyAvailableLookupTargets\n"
            );
    ...
    if ( $newinstall && isStoreWebNode() ) {
        setLdapGlobalConfig( "+zimbraReverseProxyUpstreamLoginServers",
            "$config{HOSTNAME}" );
    }
    setLdapServerConfig(
...
sub isStoreWebNode {
    if (   $installedWebapps{"zimbra"} eq "Enabled"
        || $installedWebapps{"zimbraAdmin"} eq "Enabled" )
    {
        return 1;
    }
    else {
...
sub configSetDNSCacheDefaults {
    ...
    foreach my $ip (@IPs) {
        ...
        $ip =~ s/'//g;
        $rc = main::runAsZimbra(
            "$ZMPROV ms $config{HOSTNAME} +zimbraDNSMasterIP $ip");
    }
    progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
    progress("Setting DNS cache tcp lookup preference...");
    $rc = main::runAsZimbra(
        "$ZMPROV ms $config{HOSTNAME} zimbraDNSUseTCP $config{zimbraDNSUseTCP}"
    );
    progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
    progress("Setting DNS cache udp lookup preference...");
    $rc = main::runAsZimbra(
        "$ZMPROV ms $config{HOSTNAME} zimbraDNSUseUDP $config{zimbraDNSUseUDP}"
    );
    progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
    progress("Setting DNS tcp upstream preference...");
    $rc = main::runAsZimbra(
"$ZMPROV ms $config{HOSTNAME} zimbraDNSTCPUpstream $config{zimbraDNSTCPUpstream}"
    );
    progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
    configLog("zimbraDNSCache");
}
...
sub configInitDomainAdminGroups {
    ...
    $config{zimbraDefaultDomainName} =
      getLdapConfigValue("zimbraDefaultDomainName") || $config{CREATEDOMAIN};
    ...
    my $rc =
      main::runAsZimbra( "$ZMPROV cdl $domainGroup "
    ...
    main::progress(
"Granting group $domainGroup domain right +domainAdminConsoleRights on $config{zimbraDefaultDomainName}..."
    );
    $rc = main::runAsZimbra(
"$ZMPROV grr domain $config{zimbraDefaultDomainName} grp $domainGroup +domainAdminConsoleRights"
    );
    main::progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );

    main::progress(
        "Granting group $domainGroup global right +domainAdminZimletRights...");
    $rc = main::runAsZimbra(
        "$ZMPROV grr global grp $domainGroup +domainAdminZimletRights");
    main::progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
    ...
    main::progress(
        "Granting group $domainGroup global right +adminConsoleDLRights...");
    $rc = main::runAsZimbra(
        "$ZMPROV grr global grp $domainGroup +adminConsoleDLRights");
    main::progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
...
'\(\&\(zimbraServiceHostname=\${MAILHOST}\)\(objectClass=zimbraServer\)\)';
    ...
    my @zmprov_args = ();
    push( @zmprov_args,
        ( 'zimbraReverseProxyMailHostQuery', $zimbraReverseProxyMailHostQuery )
    ) if ( getLdapConfigValue("zimbraReverseProxyMailHostQuery") eq "" );

    push( @zmprov_args,
        ( 'zimbraReverseProxyPortQuery', $zimbraReverseProxyPortQuery ) )
...
"$ZMPROV ms $config{HOSTNAME} zimbraReverseProxyStrictServerNameEnabled FALSE"
        ...
        if ( $config{MAILPROXY} eq "FALSE" && $config{HTTPPROXY} eq "FALSE" ) {
            ...
            if ( $config{MAILPROXY} eq "TRUE" ) {
                runAsZimbra(
                        "/opt/zimbra/libexec/zmproxyconfig $upstream -m -e -o "
                      . "-i $config{IMAPPORT}:$config{IMAPPROXYPORT}:$config{IMAPSSLPORT}:$config{IMAPSSLPROXYPORT} "
                      . "-p $config{POPPORT}:$config{POPPROXYPORT}:$config{POPSSLPORT}:$config{POPSSLPROXYPORT} -H $config{HOSTNAME}"
                );
            }
            ...
            if ( $config{HTTPPROXY} eq "TRUE" ) {
                runAsZimbra(
                        "/opt/zimbra/libexec/zmproxyconfig $upstream -w -e -o "
                      . " -x $config{PROXYMODE} "
                      . "-a $config{HTTPPORT}:$config{HTTPPROXYPORT}:$config{HTTPSPORT}:$config{HTTPSPROXYPORT} -H $config{HOSTNAME}"
                );
            }
    ...
    foreach my $component ( split( /\n/, $components ) ) {
        ...
        if ( $component =~ /convertd/ ) {
            ...
            if ( $result->code() ) {
                ...
                $result = $ldap->modify(
                    'cn=text/enriched,cn=mime,cn=config,cn=zimbra',
                    replace =>
                      [ 'zimbraMimeHandlerClass' => 'TextEnrichedHandler' ]
                );

                $result = $ldap->modify(
                    'cn=text/plain,cn=mime,cn=config,cn=zimbra',
                    replace =>
                      [ 'zimbraMimeHandlerClass' => 'TextPlainHandler' ]
                );

                $result = $ldap->modify(
                    'cn=all,cn=mime,cn=config,cn=zimbra',
                    changes => [
                        replace =>
                ...
                $result = $ldap->delete(
'cn=application/x-zip-compressed,cn=mime,cn=config,cn=zimbra'
                );
                $result = $ldap->delete(
                    'cn=application/zip,cn=mime,cn=config,cn=zimbra');
                $result =
                  $ldap->delete('cn=text/rtf,cn=mime,cn=config,cn=zimbra');
                $result =
                  $ldap->delete('cn=unsupported,cn=mime,cn=config,cn=zimbra');
            ...
            my $rc =
              setLdapServerConfig( $config{HOSTNAME}, '-zimbraServiceEnabled',
                'convertd' );
            progress(
                ( $rc == 0 )
                ? "done.\n"
    ...
    foreach my $zimlet (
        qw(com_zimbra_backuprestore com_zimbra_convertd com_zimbra_domainadmin com_zimbra_hsm com_zimbra_license com_zimbra_mobilesync zimbra_xmbxsearch com_zimbra_xmbxsearch com_zimbra_smime_cert_admin com_zimbra_delegatedadmin com_zimbra_smime com_zimbra_two_factor_auth)
      )
    {
        system(
            "rm -rf $config{mailboxd_directory}/webapps/service/zimlet/$zimlet")
          if (
            -d "$config{mailboxd_directory}/webapps/service/zimlet/$zimlet" );
...
sub removeNetworkZimlets {
    ...
    if ( $result->code() ) {
        ...
        $result = $ldap->search(
            ...
            scope => 'one',
            filter =>
"(|(cn=com_zimbra_backuprestore)(cn=com_zimbra_domainadmin)(cn=com_zimbra_mobilesync)(cn=com_zimbra_hsm)(cn=com_zimbra_convertd)(cn=com_zimbra_license)(cn=zimbra_xmbxsearch)(cn=com_zimbra_xmbxsearch)(cn=com_zimbra_smime)(cn=com_zimbra_smime_cert_admin)(cn=com_zimbra_two_factor_auth))",
            attrs => ['cn']
        );
        progress( ( $result->code() ) ? "failed.\n" : "done.\n" );
        ...
        foreach my $entry ( $result->all_entries ) {
            ...
            if ( $zimlet ne "" ) {
                ...
                my $rc = runAsZimbra(
                    "/opt/zimbra/bin/zmzimletctl -l undeploy $zimlet");
                progress(
                    ( $rc == 0 )
                    ? "done.\n"
                    : "failed. This may impact system functionality.\n"
...
sub zimletCleanup {
    ...
    if ( $result->code() ) {
        ...
        $result = $ldap->search(
            ...
            scope => 'one',
            filter =>
"(|(cn=convertd)(cn=hsm)(cn=hotbackup)(cn=zimbra_cert_manager)(cn=com_zimbra_search)(cn=zimbra_xmbxsearch)(cn=com_zimbra_domainadmin)(cn=com_zimbra_tinymce)(cn=com_zimbra_tasksreminder)(cn=com_zimbra_linkedin)(cn=com_zimbra_social)(cn=com_zimbra_dnd)(cn=com_zextras_chat_open))",
            attrs => ['cn']
        );
        return $result if ( $result->code() );
        detail("Processing ldap search results");
...
sub configInstallZimlets {
    ...
    my $zimlet_directory =
      getLocalConfig("zimlet_directory") || "/opt/zimbra/zimlets-deployed";
    my $zimlet_properties = getLocalConfig("zimlet_properties_directory")
      || "/opt/zimbra/zimlets-properties";
    ...
    if ( opendir DIR, "/opt/zimbra/zimlets" ) {
        ...
        my @core_zimlets = (
            qw(com_zimbra_dnd com_zimbra_url com_zimbra_date com_zimbra_email com_zimbra_attachcontacts com_zimbra_attachmail)
        );
        ...
        foreach my $zimletfile (@zimlets) {
            ...
            progress("\t$zimlet...");
            my $rc = runAsZimbra(
                "/opt/zimbra/bin/zmzimletctl -l deploy zimlets/$zimletfile");
            if ( $rc == 0 ) {
    ...
    if ( opendir DIR, "/opt/zimbra/zimlets-network" ) {
        ...
        foreach my $zimletfile (@zimlets) {
            ...
            progress("\t$zimlet...");
            my $rc = runAsZimbra(
"/opt/zimbra/bin/zmzimletctl -l deploy zimlets-network/$zimletfile"
            );
            progress(
                ( $rc == 0 )
                ? "done.\n"
            ...
            if (   ( $rc == 0 )
                && ( $zimlet eq "com_zimbra_smime" )
                && ( $config{UIWEBAPPS} eq "yes" ) )
            {
                system(
"cp /opt/zimbra/zimlets-deployed/com_zimbra_smime/com_zimbra_smime.jarx /opt/zimbra/jetty/webapps/zimbra/public/com_zimbra_smime.jarx"
                );
            }
        }
    ...
    if ( !$newinstall ) {
        ...
        if ( $result->code() ) {
            ...
            $result = $ldap->search(
                base   => $ldap_base,
                scope  => 'one',
                filter => '(objectClass=zimbraZimletEntry)',
                attrs  => ['cn']
            );
            progress( ( $result->code() ) ? "failed.\n" : "done.\n" );
            return $result if ( $result->code() );
            ...
            foreach my $entry ( $result->all_entries ) {
                my $zimlet = $entry->get_value('cn');
                foreach my $type (
                    qw(zimlets-admin-extra zimlets-experimental zimlets-extra))
                {
                    if ( -e "/opt/zimbra/${type}/${zimlet}.zip" ) {
                        progress("\t$zimlet...");
                        my $rc = runAsZimbra(
"/opt/zimbra/bin/zmzimletctl -l deploy ${type}/${zimlet}.zip"
                        );
                        progress(
                            ( $rc == 0 )
                            ? "done.\n"
...
sub configInitSql {
    ...
    if ( !$sqlConfigured && isEnabled("zimbra-store") ) {
        ...
        runAsZimbra(
"/opt/zimbra/libexec/zmmyinit --mysql_memory_percent $config{MYSQLMEMORYPERCENT}"
        );
        progress("done.\n");
        ...
        if ( isNetwork() ) {
            if ( $config{RUNVMHA} eq "yes" ) {
                push( @enabledServiceList,
                    ( 'zimbraServiceEnabled', 'vmware-ha' ) );
            }
        }
    }
    configLog("configInitCore");
...
sub configInitMta {
    ...
    if ( isEnabled("zimbra-mta") ) {
        ...
        runAsZimbra(
            "/opt/zimbra/libexec/zmmtainit $config{LDAPHOST} $config{LDAPPORT}"
        );
        progress("done.\n");
        if ( isZCS() ) {
...
sub configCreateDefaultDomainGALSyncAcct {
    ...
    if ( isEnabled("zimbra-store") ) {
        ...
        my $rc = runAsZimbra(
"/opt/zimbra/bin/zmgsautil createAccount -a $galsyncacct -n InternalGAL --domain $default_domain -s $zimbra_server -t zimbra -f _InternalGAL"
        );
        progress( ( $rc == 0 ) ? "done.\n" : "failed.\n" );
...
sub configImap {
    ...
    if ( $config{DOADDUPSTREAMIMAP} eq "yes" ) {
        progress(
"Adding $config{HOSTNAME} to list of zimbraReverseProxyUpstreamImapServers..."
        );
        runAsZimbra(
"$ZMPROV mcf +zimbraReverseProxyUpstreamImapServers $config{HOSTNAME}"
        );
        progress("done.\n");
...
sub configSetEnabledServices {
    ...
    foreach my $p ( keys %installedPackages ) {
        if ( $p eq "zimbra-core" ) {
            ...
            if ( isNetwork() ) {
                if ( -x "/usr/lib/vmware-tools/sbin64/vmware-checkvm"
                    || $config{INSTVMHA} eq "yes" )
                {
                    my $rc =
                      runAsRoot("/usr/lib/vmware-tools/sbin64/vmware-checkvm");
...
# for zimbraSSldHParam.  See bug 104244.
    ...
    setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersion',
        $curVersion );
    setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionMajor',
        $curVersionMajor );
    setLdapServerConfig( $config{HOSTNAME}, 'zimbraServerVersionMinor',
    ...
    if ( $config{STARTSERVERS} eq "yes" ) {
        ...
        if ( isEnabled("zimbra-store") ) {
            qx(chown zimbra:zimbra /opt/zimbra/redolog/redo.log)
              if ( ( $platform =~ m/DEBIAN/ || $platform =~ m/UBUNTU/ )
                && !$newinstall );
        }

        sub prevVersionBelow880 {
            if (   ( $prevVersionMajor < 8 )
                || ( $prevVersionMajor = 8 && $prevVersionMinor < 8 ) )
            {
                return 1;
            }
        }
        ...
        if ( isInstalled("zimbra-network-modules-ng") && $newinstall ) {
            ...
            setLdapServerConfig( $config{HOSTNAME},
                'zimbraNetworkMobileNGEnabled', 'TRUE' );
            setLdapServerConfig( $config{HOSTNAME},
...
sub zxsuiteIsAvailable {
    ...
    if ( ( -f "/opt/zimbra/bin/zxsuite" ) && ( $output =~ /valid(.*)true/ ) ) {
        $NGbackup = "true";
        detail(
"NG backup is already initialized because /opt/zimbra/bin/zxsuite backup getBackupInfo valid has value: $NGbackup \n"
        );
    }
    else {
        $NGbackup = "false";
        detail(
"Modifying the crontab with default schedule because \"/opt/zimbra/bin/zxsuite backup getBackupInfo\" valid has value: $NGbackup \n"
        );
    }
    return $NGbackup;
}
...
sub setupCrontab {
    ...
    $nohsm =
      0xffff &
      system(
"grep '/opt/zimbra/bin/zmhsm[[:space:]]\\+-t' /tmp/crontab.zimbra.orig > /dev/null 2>&1"
      );
    if ( !$nohsm ) {
    ...
    my $rc = 0xffff &
      system("grep ZIMBRASTART /tmp/crontab.zimbra.orig > /dev/null 2>&1");
    if ($rc) {
        detail(
"crontab: ZIMBRASTART not found truncating zimbra crontab and starting fresh."
        );
        qx(cp -f /dev/null /tmp/crontab.zimbra.orig 2>> $logfile);
    }
    detail("crontab: Looking for ZIMBRAEND in existing crontab entry.");
    $rc = 0xffff &
      system("grep ZIMBRAEND /tmp/crontab.zimbra.orig > /dev/null 2>&1");
    if ($rc) {
        detail(
"crontab: ZIMBRAEND not found truncating zimbra crontab and starting fresh."
        );
        qx(cp -f /dev/null /tmp/crontab.zimbra.orig);
    }
    detail(
        "crontab: Getting existing backup and custom entries from crontab file."
    );
...
qx(echo "# ZIMBRAEND -- DO NOT EDIT ANYTHING BETWEEN THIS LINE AND ZIMBRASTART" >> /tmp/crontab.zimbra);
    ...
    if ( -x "/opt/zimbra/bin/zmschedulebackup" && scalar @backupSchedule > 0 ) {
        ...
        for ( my $i = 0 ; $i <= $#backupSchedule ; $i++ ) {
            ...
            if ( $i == 0 ) {
                detail(
"crontab: $SU \"/opt/zimbra/bin/zmschedulebackup -R $backupSchedule[$i]\""
                );
                runAsZimbra(
                    "/opt/zimbra/bin/zmschedulebackup -R $backupSchedule[$i]");
            }
            else {
                detail(
"crontab: $SU \"/opt/zimbra/bin/zmschedulebackup -A $backupSchedule[$i]\""
                );
                runAsZimbra(
                    "/opt/zimbra/bin/zmschedulebackup -A $backupSchedule[$i]");
            }
        }
    }
    elsif (-f "/opt/zimbra/bin/zmschedulebackup"
        && scalar @backupSchedule == 0
        && !$newinstall
        && $nohsm
        && $NG_backup == "false" )
    {
        detail(
            "crontab: No backup schedule found: installing default schedule.");
...
sub addServerToHostPool {
    progress(
        "Adding $config{HOSTNAME} to zimbraMailHostPool in default COS...");
