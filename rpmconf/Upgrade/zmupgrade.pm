...
my $lowVersion = 52;
my $hiVersion =
  112;    # this should be set to the DB version expected by current server code
...
my %updateScripts = (
    '65' =>
      "migrate20120611_7to8_bundle.pl",   # this upgrades to 90 for 8_0_0_BETA
    ...
    '111' => "migrate20180613-EventFlagColumn.pl",
);
...
my (
    $targetVersion, $targetMajor,      $targetMinor,
    $targetMicro,   $targetMicroMicro, $targetType
);

sub applicableVersions {
    my $versionHash = shift;
    my @versionOrder;
    foreach my $key ( grep { !( version_cmp( $_, $startVersion ) < 0 ) }
        sort { version_cmp( $a, $b ); } keys %$versionHash )
    {
...
my @packageList = (
    "zimbra-core", "zimbra-ldap",   "zimbra-store",  "zimbra-mta",
    "zimbra-snmp", "zimbra-logger", "zimbra-apache", "zimbra-spell",
);
...
sub upgrade {
    ...
    ( $startVersion, $startBuild ) =
      $startVersion =~ /(\d\.\d\.\d+_[^_]*)_(\d+)/;
    ( $targetVersion, $targetBuild ) =
      $targetVersion =~ m/(\d\.\d\.\d+_[^_]*)_(\d+)/;
...
# Bug #73840 - need to delete /opt/zimbra/keyview before we try stopping services
    ...
    if ($startVersion) {
        ...
        main::progress(
            "ERROR: Unable to find initial version to upgrade from.\n");
        main::progress(
"       This indicates a corrupted /opt/zimbra/.install_history file.\n"
        );
        main::progress(
            "       DO NOT ATTEMPT UPGRADING AGAIN UNTIL THE FILE IS FIXED.\n");
        return 1;
    }
...
# temporary symlinks for relocation of key mysql files
            ...
            symlink( "/opt/zimbra/db/mysql.sock",
    ...
    if ( main::isInstalled("zimbra-store") ) {
        ...
        if ( $curSchemaVersion < $hiVersion ) {
            main::progress(
"Schema upgrade required from version $curSchemaVersion to $hiVersion.\n"
            );
        }

        # the old slow painful way (ie lots of mysql invocations)
        while ($curSchemaVersion >= $lowVersion
            && $curSchemaVersion < $hiVersion )
        {
    ...
    if (
        $startVersion ne $targetVersion   # if you are upgrading across versions
        || $targetBuild > $startBuild
      )    # or you are upgrading across new builds of the same version
    {
        ...
        foreach my $v ( @{ applicableVersions( \%updateFuncs ) } ) {
            ...
            if ( &{ $updateFuncs{$v} }
                ( $startBuild, $targetVersion, $targetBuild ) )
            {
                main::progress(
                    "Something failed while applying updates for $v - exiting\n"
                );
                return 1;
            }
        }
    ...
    if ($isLdapMaster) {
        main::progress(
"Updating global config and COS's with attributes introduced after $startVersion..."
        );
        main::progress(
            ( &runAttributeUpgrade($startVersion) ) ? "failed.\n" : "done.\n" );
...
sub upgrade701GA {
    ...
    if ( main::isInstalled("zimbra-store") ) {
        ...
        if ( !fgrep { /^max_allowed_packet/ } ${mysql_mycnf} ) {
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-allowed-packet --section=mysqld --key=max_allowed_packet --set --value=16777216 ${mysql_mycnf}"
            );
        }
        if ( -d "/opt/zimbra/data/mailboxd/imap/cache" ) {
            system(
"/bin/rm -rf /opt/zimbra/data/mailboxd/imap/cache/* 2> /dev/null"
            );
        }
    }
    return 0;
...
sub upgrade710GA {
    ...
    if ( main::isInstalled("zimbra-store") ) {
        ...
        main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-log_file_size --section=mysqld --key=innodb_log_file_size --set --value=524288000 ${mysql_mycnf}"
        );
        main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-dirty-pages --section=mysqld --key=innodb_max_dirty_pages_pct --set --value=30 ${mysql_mycnf}"
        );
        ...
        foreach my $attr (@attrvals) {
            if ( $attr =~ /zimbraDomainName/ ) {
                ...
                if ( $attr !~ /uid=zmamavis,cn=appaccts,cn=zimbra/ ) {
                    ...
                    if ( $attrMod =~ /by \* read/ ) {
                        $attrMod =~
s/by \* read/by dn.base="uid=zmamavis,cn=appaccts,cn=zimbra" read  by \* read/;
                    }
                    else {
                        $attrMod =~
s/by \* none/by dn.base="uid=zmamavis,cn=appaccts,cn=zimbra" read  by \* none/;
                    }
                }
            }
        }
        ...
        if ( $aclNumber != -1 && $attrMod ne "" ) {
            $result =
              $ldap->modify( $dn, delete => { olcAccess => "{$aclNumber}" }, );
            $result = $ldap->modify( $dn, add => { olcAccess => "$attrMod" }, );
        }
        $result = $ldap->search(
            base   => "$dn",
            filter => "(objectClass=*)",
        ...
        if ( $aclNumber != -1 && $attrMod ne "" ) {
            if ($fixup) {
                $attrMod =~
s/homePhone,mobile,pager/homePhone,pager,mobile,userCertificate/;
            }
            else {
                $attrMod =~
s/homePhone,pager,mobile/homePhone,pager,mobile,userCertificate/;
            }
            $result =
              $ldap->modify( $dn, delete => { olcAccess => "{$aclNumber}" }, );
            $result = $ldap->modify( $dn, add => { olcAccess => "$attrMod" }, );
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        ...
        unless ( $result->code() ) {
            $result = $ldap->modify( "cn=config",
                add => { 'olcTLSCACertificatePath' => '/opt/zimbra/conf/ca' } );
        }
        $result = $ldap->unbind;
    }
    if ( main::isInstalled("zimbra-mta") ) {
...
"$ZMPROV mcf +zimbraMtaRestriction reject_invalid_helo_hostname"
            ...
            if ( $restriction =~ /^reject_non_fqdn_hostname$/ ) {
                ...
                main::runAsZimbra(
"$ZMPROV mcf +zimbraMtaRestriction reject_non_fqdn_helo_hostname"
                );
            }
...
"$ZMPROV mcf +zimbraMtaRestriction reject_unknown_client_hostname"
            ...
            if ( $restriction =~ /^reject_unknown_hostname$/ ) {
                main::runAsZimbra(
                    "$ZMPROV mcf -zimbraMtaRestriction reject_unknown_hostname"
                );
                main::runAsZimbra(
"$ZMPROV mcf +zimbraMtaRestriction reject_unknown_helo_hostname"
                );
            }
        }
    }
...
sub upgrade720GA {
    ...
    if ( main::isInstalled("zimbra-store") ) {
        ...
        if ( -d "/opt/zimbra/zimlets-deployed/com_zimbra_smime/" ) {
            main::runAsZimbra(
                "/opt/zimbra/bin/zmzimletctl -l undeploy com_zimbra_smime");
            system(
"rm -rf /opt/zimbra/mailboxd/webapps/service/zimlet/com_zimbra_smime"
              )
              if (
                -d "/opt/zimbra/mailboxd/webapps/service/zimlet/com_zimbra_smime"
              );
        }
    }
...
sub upgrade800BETA1 {
    ...
    foreach my $lc_var (
        qw(enable_config_restarts interval log_level listen_port debug watchdog watchdog_services)
      )
    {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        ...
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20110615-AddDynlist.pl");
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20110721-AddUnique.pl");
        my $ldap_pass = qx($su "zmlocalconfig -s -m nokey ldap_root_password");
        chomp($ldap_pass);
        my $ldap;
        unless (
...
sub upgrade800BETA2 {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        ...
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20111019-UniqueZimbraId.pl"
        );
    }
    if ( main::isEnabled("zimbra-store") ) {
        if ( startSql() ) { return 1; }
        main::runAsZimbra(
"perl -I${scriptDir} ${scriptDir}/migrate20111005-ItemIdCheckpoint.pl"
        );

        # Bug: 60011
        my $mysql_root_password =
          qx(/opt/zimbra/bin/zmlocalconfig -s -x -m nokey mysql_root_password);
        my $mysql_socket =
          qx(/opt/zimbra/bin/zmlocalconfig -s -x -m nokey mysql_socket);
        my $host = qx(hostname);
...
qx(/opt/zimbra/common/bin/mysql -S '$mysql_socket' -u root --password='$mysql_root_password' -e "DROP USER ''\@'localhost'; DROP USER ''\@'${host}'");
    ...
    if ( main::isInstalled("zimbra-proxy") ) {
        ...
        if ( main::isEnabled("zimbra-proxy") ) {
            main::setLdapServerConfig( $hn, '-zimbraServiceEnabled',
                'imapproxy' );
...
sub upgrade800BETA3 {
    ...
    main::setLocalConfig( "ldap_read_timeout", "0" );    #70437
    main::detail(
"Removing /opt/zimbra/ssl/zimbra/{ca,server} to force creation or download of new ca and certificates."
    );
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        ...
        foreach my $lc_var (
            qw(ldap_db_cachefree ldap_db_cachesize ldap_db_dncachesize ldap_db_idlcachesize ldap_db_shmkey ldap_overlay_syncprov_sessionlog)
          )
        {
        ...
        foreach my $lc_var (
            qw(ldap_accesslog_cachefree ldap_accesslog_cachesize ldap_accesslog_dncachesize ldap_accesslog_idlcachesize ldap_accesslog_shmkey)
          )
        {
            my $val = main::getLocalConfig("${lc_var}");
            if ( $val ne "" ) {
        ...
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20120210-AddSearchNoOp.pl"
        );
    }
    if ( main::isInstalled("zimbra-store") ) {
...
qx(cp -pf /opt/zimbra/jetty-6.1.22.z6/etc/jetty.keytab /opt/zimbra/data/mailboxd/spnego/jetty.keytab);
    ...
    if ( main::isInstalled("zimbra-octopus") ) {
        if ( startSql() ) { return 1; }
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20120209-octopusEvent.pl");
        stopSql();
    }
...
sub upgrade800BETA4 {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        ...
        if ( $aclNumber != -1 && $attrMod ne "" ) {
            $attrMod =~
s/zimbraAllowFromAddress/zimbraAllowFromAddress,DKIMIdentity,DKIMSelector,DKIMDomain,DKIMKey/;
            $result =
              $ldap->modify( $dn, delete => { olcAccess => "{$aclNumber}" }, );
            $result = $ldap->modify( $dn, add => { olcAccess => "$attrMod" }, );
        }
        ...
        main::runAsZimbra(
"perl -I${scriptDir} ${scriptDir}/migrate20120507-UniqueDKIMSelector.pl"
        );
    }
    if ( main::isInstalled("zimbra-proxy") ) {

        # bug 32683
        main::setLdapGlobalConfig( "zimbraReverseProxySSLToUpstreamEnabled",
            "FALSE" );
    }
    foreach my $lc_var (
        qw(cbpolicyd_bind_host logger_mysql_bind_address logger_mysql_directory logger_mysql_data_directory logger_mysql_socket logger_mysql_pidfile logger_mysql_mycnf logger_mysql_errlogfile logger_mysql_port zimbra_logger_mysql_password)
      )
    {
        main::deleteLocalConfig("$lc_var");
    }
...
sub upgrade800GA {
    ...
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( -f $cbpdb ) {
            main::runAsZimbra(
"sqlite3 $cbpdb < ${scriptDir}/migrate20130227-UpgradeCBPolicyDSchema.sql >/dev/null 2>&1"
            );
            main::runAsZimbra(
"sqlite3 $cbpdb < ${scriptDir}/migrate20130819-UpgradeQuotasTable.sql >/dev/null 2>&1"
            );
        }
    }
    return 0;
...
sub upgrade801GA {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        ...
        if ( $isLdapMaster && $alog == 1 ) {
            ...
            if ( !(@attrvals) ) {
                $result = $ldap->modify( "olcDatabase={2}mdb,cn=config",
                    add => { olcDbEnvFlags => [ "writemap", "nometasync" ] }, );
            }
        }
        $ldap->unbind;
...
sub upgrade802GA {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        if ($isLdapMaster) {
            my $ldap_pass =
              qx($su "zmlocalconfig -s -m nokey ldap_root_password");
            chomp($ldap_pass);
            my $ldap;
            unless (
                $ldap = Net::LDAP->new(
    ...
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( $mtamilter ne "" ) {
            if ( $mtamilter =~ /$miltervalue/ ) {
                ...
                main::setLdapServerConfig( "zimbraMtaSmtpdMilters",
                    "$mtamilter" );
            }
        }
    }
...
sub upgrade803GA {
    ...
    if ( main::isInstalled("zimbra-store") ) {
        my $mailboxd_java_options =
          main::getLocalConfigRaw("mailboxd_java_options");
        if ( $mailboxd_java_options =~ /-XX:MaxPermSize=128m/ ) {
            $mailboxd_java_options =~
              s/-XX:MaxPermSize=128m/-XX:MaxPermSize=350m/;
            main::detail(
                "Modified mailboxd_java_options=$mailboxd_java_options");
...
sub upgrade804GA {
    ...
    if ( main::isInstalled("zimbra-store") ) {
        ...
        if ( $zimbraIPMode eq "ipv4" ) {
            ...
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-bind --section=mysqld --key=bind-address --unset ${mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-bind --section=mysqld --key=bind-address --set --value=127.0.0.1 ${mysql_mycnf}"
            );
        }
...
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-bind --section=mysqld --key=bind-address --unset ${mysql_mycnf}"
            ...
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-bind --section=mysqld --key=bind-address --set --value=::1 ${mysql_mycnf}"
            );
        }
...
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-bind --section=mysqld --key=bind-address --set --value=::1 ${mysql_mycnf}"
        ...
        my $mailboxd_java_options =
          main::getLocalConfigRaw("mailboxd_java_options");
        if ( $mailboxd_java_options !~
            /-Dorg.apache.jasper.compiler.disablejsr199/ )
        {
            $mailboxd_java_options = $mailboxd_java_options
...
sub upgrade805GA {
    ...
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( -f $cbpdb ) {
            main::runAsZimbra(
"sqlite3 $cbpdb < ${scriptDir}/migrate20130606-UpdateCBPolicydSchema.sql >/dev/null 2>&1"
            );
        }
    }
    if ( main::isInstalled("zimbra-proxy") ) {
        my $rpeioa = main::getLocalConfig(
            "zimbra_reverseproxy_externalroute_include_original_authusername");
        if ( lc($rpeioa) eq "true" ) {
            main::setLdapGlobalConfig(
...
sub upgrade806GA {
    ...
    my @zimbraStatThreadNamePrefix =
      qx($su "$ZMPROV gacf zimbraStatThreadNamePrefix");
...
sub upgrade808GA {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        my $ldap_common_writetimeout =
          main::getLocalConfig("ldap_common_writetimeout");
        if ( $ldap_common_writetimeout == 0 ) {
            main::deleteLocalConfig("ldap_common_writetimeout");    #85299
        }
    }
    if ( main::isInstalled("zimbra-mta") ) {
        my @zimbraServiceInstalled =
          qx($su "$ZMPROV gs $hn zimbraServiceInstalled");
        my @zimbraServiceEnabled =
          qx($su "$ZMPROV gs $hn zimbraServiceEnabled");
        if (   grep( /antivirus/, @zimbraServiceInstalled )
            || grep( /antispam/,  @zimbraServiceInstalled )
            || grep( /archiving/, @zimbraServiceInstalled ) )
        {
            main::setLdapServerConfig( $hn, '+zimbraServiceInstalled',
                'amavis' );
        }
        if (   grep( /antivirus/, @zimbraServiceEnabled )
            || grep( /antispam/,  @zimbraServiceEnabled )
            || grep( /archiving/, @zimbraServiceEnabled ) )
        {
...
sub upgrade850BETA1 {
        ...
        if ( $mailboxd_java_options =~ /-XX:\+PrintGCTimeStamps/ ) {
            foreach my $option ( split( /\s+/, $mailboxd_java_options ) ) {
                $new_mailboxd_options .= " $option"
                  if ( $option !~ /^-XX:\+PrintGCTimeStamps/ );
            }
            $new_mailboxd_options .= " -XX:+PrintGCDateStamps"
              unless ( $mailboxd_java_options =~ /PrintGCDateStamps/ );
            $new_mailboxd_options =~ s/^\s+//;
            main::setLocalConfig( "mailboxd_java_options",
                $new_mailboxd_options )
              if ( $new_mailboxd_options ne "" );
        }
        if ( main::isNetwork() ) {
            my @zimbraReverseProxyUpstreamEwsServers =
              qx($su "$ZMPROV gacf zimbraReverseProxyUpstreamEwsServers");
            if ( !grep( /$hn/, @zimbraReverseProxyUpstreamEwsServers ) ) {
                main::runAsZimbra(
                    "$ZMPROV mcf +zimbraReverseProxyUpstreamEwsServers $hn");
            }
        }
    ...
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( -e ${antispam_mysql_mycnf} ) {
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-as-table_cache-fixup --section=mysqld --key=table_cache --unset ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-as-table_open_cache-fixup --section=mysqld --key=table_open_cache --setmin --value=1200 ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-innodb_data_file_path-fixup --section=mysqld --set --key=innodb_data_file_path --value=ibdata1:10M:autoextend ${antispam_mysql_mycnf}"
            );
        }
        my $disclaimerEnabled =
          main::getLdapConfigValue("zimbraDomainMandatoryMailSignatureEnabled");
        if ( lc($disclaimerEnabled) eq "true" ) {
            ...
            foreach my $domain (@domains) {
                chomp $domain;
                main::runAsZimbra(
                    "/opt/zimbra/libexec/zmaltermimeconfig -e $domain");
            }
        }
        ...
        if ( defined($lc_attr) && $lc_attr + 0 != 0 ) {
            main::setLdapServerConfig( $hn, 'zimbraAmavisMaxServers',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{clamav_max_threads}->{value};
        if ( defined($lc_attr) && $lc_attr + 0 != 0 ) {
            main::setLdapServerConfig( $hn, 'zimbraClamAVMaxThreads',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{amavis_enable_dkim_verification}->{value};
        if ( defined($lc_attr) && lc($lc_attr) eq "false" ) {
            main::setLdapServerConfig( $hn,
                'zimbraAmavisEnableDKIMVerification', "FALSE" );
        }
        $lc_attr = $localxml->{key}->{amavis_originating_bypass_sa}->{value};
        if ( defined($lc_attr) && lc($lc_attr) eq "true" ) {
            main::setLdapServerConfig( $hn, 'zimbraAmavisOriginatingBypassSA',
                "TRUE" );
        }
        $lc_attr = $localxml->{key}->{amavis_dspam_enabled}->{value};
        if ( defined($lc_attr)
            && ( lc($lc_attr) eq "yes" || lc($lc_attr) eq "true" ) )
        {
            main::setLdapServerConfig( $hn, 'zimbraAmavisDSPAMEnabled',
                "TRUE" );
        }
        ...
        {
            main::setLdapServerConfig( $hn,
                'zimbraCBPolicydGreylistingEnabled', "TRUE" );
        }
        $lc_attr =
          $localxml->{key}->{cbpolicyd_module_greylisting_training}->{value};
        if ( $lc_attr ne "" && ( 0 + $lc_attr > 0 || lc($lc_attr) eq "true" ) )
        {
            main::setLdapServerConfig( $hn,
                'zimbraCBPolicydGreylistingTrainingEnabled', "TRUE" );
        }
        $lc_attr =
          $localxml->{key}->{cbpolicyd_module_greylisting_defer_msg}->{value};
        if ( defined($lc_attr) ) {
            main::setLdapServerConfig( $hn,
                'zimbraCBPolicydGreylistingDeferMsg', "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{cbpolicyd_module_greylisting_blacklist_msg}
          ->{value};
        if ( defined($lc_attr) ) {
            main::setLdapServerConfig( $hn,
                'zimbraCBPolicydGreylistingBlacklistMsg', "$lc_attr" );
        }
        ...
        {
            main::setLdapServerConfig( $hn, 'zimbraCBPolicydCheckHeloEnabled',
                "TRUE" );
        }
        ...
        {
            main::setLdapServerConfig( $hn, 'zimbraCBPolicydCheckSPFEnabled',
                "TRUE" );
        }
        ...
        {
            main::setLdapServerConfig( $hn, 'zimbraCBPolicydQuotasEnabled',
                "FALSE" );
        }
        ...
        {
            main::setLdapServerConfig( $hn, 'zimbraCBPolicydAmavisEnabled',
                "TRUE" );
        }
        ...
        {
            main::setLdapServerConfig( $hn, 'zimbraCBPolicydAccountingEnabled',
                "TRUE" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_always_add_missing_headers}->{value};
        if ( defined($lc_attr) && lc($lc_attr) eq "no" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaAlwaysAddMissingHeaders',
                "no" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_broken_sasl_auth_clients}->{value};
        if ( defined($lc_attr) && lc($lc_attr) eq "no" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaBrokenSaslAuthClients',
                "no" );
        }
        $lc_attr = $localxml->{key}->{postfix_bounce_notice_recipient}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "postmaster" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaBounceNoticeRecipient',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_bounce_queue_lifetime}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "5d" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaBounceQueueLifetime',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_delay_warning_time}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "0h" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaDelayWarningTime',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn, '+zimbraMtaHeaderChecks',
                    "$option" );
            }
        }
        $lc_attr = $localxml->{key}->{postfix_in_flow_delay}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "1s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaInFlowDelay',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn, '+zimbraMtaImportEnvironment',
                    "$option" );
            }
        }
        $lc_attr =
          $localxml->{key}->{postfix_lmtp_connection_cache_destinations}
          ->{value};
        if ( defined($lc_attr) ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn,
                    '+zimbraMtaLmtpConnectionCacheDestinations', "$option" );
            }
        }
        $lc_attr =
          $localxml->{key}->{postfix_lmtp_connection_cache_time_limit}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "4s" ) {
            main::setLdapServerConfig( $hn,
                'zimbraMtaLmtpConnectionCacheTimeLimit', "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) && lc($lc_attr) ne "dns" ) {
            ...
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn, '+zimbraMtaLmtpHostLookup',
                    "$option" );
            }
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            main::setLdapServerConfig( $hn, 'zimbraMtaQueueDirectory',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_maximal_backoff_time}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "4000s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaMaximalBackoffTime',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_minimal_backoff_time}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "300s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaMinimalBackoffTime',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_queue_run_delay}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "300s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaQueueRunDelay',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_milter_connect_timeout}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "30s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaMilterConnectTimeout',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_milter_content_timeout}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "300s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaMilterContentTimeout',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_milter_default_action}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "tempfail" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaMilterDefaultAction',
                "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtp_cname_overrides_servername}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "no" ) {
            main::setLdapServerConfig( $hn,
                'zimbraMtaSmtpCnameOverridesServername', "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtp_helo_name}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne '$myhostname' ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpHeloName',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtp_sasl_auth_enable}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "no" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpSaslAuthEnable',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtp_tls_security_level}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "may" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpTlsSecurityLevel',
                "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtp_sasl_mechanism_filter}->{value};
        if ( defined($lc_attr) ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn,
                    '+zimbraMtaSmtpSaslMechanismFilter', "$option" );
            }
        }
        ...
        if ( defined($lc_attr) ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpSaslPasswordMaps',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_policy_time_limit}->{value};
        if ( defined($lc_attr) ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaPolicyTimeLimit',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_banner}->{value};
        if ( defined($lc_attr) ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdBanner',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_proxy_timeout}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "100s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdProxyTimeout',
                "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtpd_reject_unlisted_recipient}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "no" ) {
            main::setLdapServerConfig( $hn,
                'zimbraMtaSmtpdRejectUnlistedRecipient', "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtpd_reject_unlisted_sender}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "no" ) {
            main::setLdapServerConfig( $hn,
                'zimbraMtaSmtpdRejectUnlistedSender', "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtpd_sasl_authenticated_header}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "no" ) {
            main::setLdapServerConfig( $hn,
                'zimbraMtaSmtpdSaslAuthenticatedHeader', "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_hard_error_limit}->{value};
        if ( defined($lc_attr) && $lc_attr != 20 ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdHardErrorLimit',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_soft_error_limit}->{value};
        if ( defined($lc_attr) && $lc_attr != 10 ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdSoftErrorLimit',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_error_sleep_time}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "1s" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdErrorSleepTime',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_helo_required}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "yes" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdHeloRequired',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_tls_loglevel}->{value};
        if ( defined($lc_attr) && $lc_attr != 1 ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdTlsLoglevel',
                "$lc_attr" );
        }
        $lc_attr =
          $localxml->{key}->{postfix_virtual_alias_expansion_limit}->{value};
        if ( defined($lc_attr) && $lc_attr != 10000 ) {
            main::setLdapServerConfig( $hn,
                'zimbraMtaVirtualAliasExpansionLimit', "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_virtual_transport}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "error" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdVirtualTransport',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) && lc($lc_attr) ne "resource,software" ) {
            ...
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn, '+zimbraMtaNotifyClasses',
                    "$option" );
            }
        }
        $lc_attr =
          $localxml->{key}->{postfix_propagate_unmatched_extensions}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "canonical" ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn,
                    '+zimbraMtaPropagateUnmatchedExtensions', "$option" );
            }
        }
        ...
        $lc_attr =
          $localxml->{key}->{postfix_smtp_sasl_security_options}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "noplaintext,noanonymous" ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn,
                    '+zimbraMtaSmtpSaslSecurityOptions', "$option" );
            }
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtpd_sasl_security_options}->{value};
        if ( defined($lc_attr) && lc($lc_attr) ne "noplaintext,noanonymous" ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn,
                    '+zimbraMtaSmtpdSaslSecurityOptions', "$option" );
            }
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtpd_sasl_tls_security_options}->{value};
        if ( defined($lc_attr) ) {
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn,
                    '+zimbraMtaSmtpdSaslTlsSecurityOptions', "$option" );
            }
        }
        $lc_attr =
          $localxml->{key}->{postfix_smtpd_client_restrictions}->{value};
        if ( defined($lc_attr) ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdClientRestrictions',
                "$lc_attr" );
        }
        $lc_attr = $localxml->{key}->{postfix_smtpd_data_restrictions}->{value};
        if ( defined($lc_attr) ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpdDataRestrictions',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            main::setLdapServerConfig( $hn, 'zimbraMtaTransportMaps',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            main::setLdapServerConfig( $hn, 'zimbraMtaVirtualAliasDomains',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            main::setLdapServerConfig( $hn, 'zimbraMtaVirtualAliasMaps',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            main::setLdapServerConfig( $hn, 'zimbraMtaVirtualMailboxDomains',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/, /,/g;
            $lc_attr =~ s/\s+/ /g;
            main::setLdapServerConfig( $hn, 'zimbraMtaVirtualMailboxMaps',
                "$lc_attr" );
        }
        ...
        if ( defined($lc_attr) ) {
            ...
            $lc_attr =~ s/\s+/ /g;
            foreach my $option ( split( /,|\s/, $lc_attr ) ) {
                main::setLdapServerConfig( $hn, '+zimbraMtaSaslSmtpdMechList',
                    "$option" );
            }
        }
...
sub upgrade850BETA2 {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        if ($isLdapMaster) {
            main::setLdapGlobalConfig( "zimbraVersionCheckURL",
    ...
    if ( main::isInstalled("zimbra-store") ) {
        ...
        if ( -e ${mysql_mycnf} ) {
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-general_log_file-fixup --section=mysqld --set --key=general_log_file --value=${zimbra_log_directory}/mysql-mailboxd.log ${mysql_mycnf}"
            );
        }
        my $mailboxd_java_options =
          main::getLocalConfigRaw("mailboxd_java_options");
...
" -Xloggc:/opt/zimbra/log/gc.log -XX:-UseGCLogFileRotation -XX:NumberOfGCLogFiles=20 -XX:GCLogFileSize=4096K";
        ...
        if ( main::isStoreWebNode() ) {
            my @zimbraReverseProxyUpstreamLoginServers =
              qx($su "$ZMPROV gacf zimbraReverseProxyUpstreamLoginServers");
            if ( !grep( /$hn/, @zimbraReverseProxyUpstreamLoginServers ) ) {
                main::runAsZimbra(
                    "$ZMPROV mcf +zimbraReverseProxyUpstreamLoginServers $hn");
            }
        }
    }
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( -e ${antispam_mysql_mycnf} ) {
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-antispam-general_log_file-fixup --section=mysqld --set --key=general_log_file --value=${zimbra_log_directory}/mysql-antispam.log ${antispam_mysql_mycnf}"
            );
        }
        my @zimbraServiceInstalled =
          qx($su "$ZMPROV gs $hn zimbraServiceInstalled");
...
sub upgrade850BETA3 {
    ...
    if ( main::isInstalled("zimbra-store") ) {
        if ( main::isStoreServiceNode() ) {
            my @zimbraReverseProxyAvailableLookupTargets =
              qx($su "$ZMPROV gacf zimbraReverseProxyAvailableLookupTargets");
            if ( !grep( /$hn/, @zimbraReverseProxyAvailableLookupTargets ) ) {
                main::runAsZimbra(
                    "$ZMPROV mcf +zimbraReverseProxyAvailableLookupTargets $hn"
                );
            }
        }
    }
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( -e ${antispam_mysql_mycnf} ) {
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-long-query-time-fixup --section=mysqld --unset --key=long-query-time ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-long_query_time-fixup --section=mysqld --set --key=long_query_time --value=1 ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-log-queries-not-using-indexes-fixup --section=mysqld --unset --key=log-queries-not-using-indexes ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-log_queries_not_using_indexes-fixup --section=mysqld --set --key=log_queries_not_using_indexes ${antispam_mysql_mycnf}"
            );
        }
    }
    return 0;
...
sub upgrade850GA {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20140728-AddSSHA512.pl");
        if ($isLdapMaster) {
            main::runAsZimbra(
                "$ZMPROV mcf +zimbraSpamTrashAlias '/Deleted Messages'");
            main::runAsZimbra(
                "$ZMPROV mcf +zimbraSpamTrashAlias '/Deleted Items'");
        }
    }
    return 0;
}
...
'ldapi://%2fopt%2fzimbra%2fdata%2fldap%2fstate%2frun%2fldapi/'
              ...
              )
            {
                ...
                $result = $ldap->search(
                    ...
                    filter =>
"(&(objectClass=*)(zimbraDomainMandatoryMailSignatureText=*))",
                    scope => "base",
                    attrs => [
                ...
                if ( $totalcount > 0 ) {
                    ...
                    my $text_disclaimer = $entry->get_value(
                        "zimbraDomainMandatoryMailSignatureText");
                    my $html_disclaimer = $entry->get_value(
                        "zimbraDomainMandatoryMailSignatureHTML");
                    $result = $ldap->search(
                        base   => "",
...
sub upgrade860BETA2 {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        main::runAsZimbra(
            "perl -I${scriptDir} ${scriptDir}/migrate20141022-AddTLSBits.pl");
    }
    if ( main::isInstalled("zimbra-store") ) {
        my @zimbraHttpContextPathBasedThreadPoolBalancingFilterRules =
qx($su "$ZMPROV gacf zimbraHttpContextPathBasedThreadPoolBalancingFilterRules");
        foreach my $zimbraHttpContextPathBasedThreadPoolBalancingFilterRule (
            @zimbraHttpContextPathBasedThreadPoolBalancingFilterRules)
        {
            chomp($zimbraHttpContextPathBasedThreadPoolBalancingFilterRule);
            ( my $filterKey, my $filterValue ) = split( /:\s/,
                $zimbraHttpContextPathBasedThreadPoolBalancingFilterRule );
            if ( $filterValue eq "/service:min=10;max=80%" ) {
                main::runAsZimbra(
"$ZMPROV mcf -zimbraHttpContextPathBasedThreadPoolBalancingFilterRules '$filterValue'"
                );
                main::runAsZimbra(
"$ZMPROV mcf +zimbraHttpContextPathBasedThreadPoolBalancingFilterRules '/service:max=80%'"
                );
            }
...
"$ZMPROV mcf -zimbraHttpContextPathBasedThreadPoolBalancingFilterRules '$filterValue'"
                ...
                main::runAsZimbra(
"$ZMPROV mcf +zimbraHttpContextPathBasedThreadPoolBalancingFilterRules '/zimbra:max=15%'"
                );
            }
...
"$ZMPROV mcf -zimbraHttpContextPathBasedThreadPoolBalancingFilterRules '$filterValue'"
                ...
                main::runAsZimbra(
"$ZMPROV mcf +zimbraHttpContextPathBasedThreadPoolBalancingFilterRules '/zimbraAdmin:max=5%'"
                );
            }
        }
    }
...
sub upgrade860GA {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        if ($isLdapMaster) {
            my $mtasmtpdprotocols =
              main::getLdapConfigValue("zimbraMtaSmtpdTlsProtocols");
    ...
    if ( -d "/opt/zimbra/zimlets-deployed/com_zimbra_linkedinimage/" ) {
        main::runAsZimbra(
            "/opt/zimbra/bin/zmzimletctl -l undeploy com_zimbra_linkedinimage");
    }
    return 0;
}
...
sub upgrade870BETA1 {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        if ($isLdapMaster) {
            ...
            main::runAsZimbra(
"perl -I${scriptDir} ${scriptDir}/migrate20150930-AddSyncpovSessionlog.pl"
            );

            # Bug 96921 - Update Jetty default SSL cipher excludes...
            my $sslexcludeciph =
              main::getLdapConfigValue("zimbraSSLExcludeCipherSuites") || "";
            ...
            if ( $cursslexcl eq $oldsslexcl ) {
                main::runAsZimbra(
                    "$ZMPROV mcf zimbraSSLExcludeCipherSuites '.*_RC4_.*'");
            }

            # Bug 97332 - Some clients require SSLv2Hello support...
            my $sslprot =
              main::getLdapConfigValue("zimbraMailboxdSSLProtocols") || "";
    ...
    if ( main::isInstalled("zimbra-proxy") ) {
        my $proxysslciphers =
          main::getLdapConfigValue("zimbraReverseProxySSLCiphers");
        if ( $proxysslciphers eq
...
"Updating mailboxd_java_options to remove deprecated PermSize and MaxPermSize java options.\n"
        ...
        my $proxy_reconnect_timeout =
          main::getLdapServerValue("zimbraMailProxyReconnectTimeout");
        if ( $proxy_reconnect_timeout eq "60" ) {
            main::setLdapServerConfig( $hn, 'zimbraMailProxyReconnectTimeout',
                '10' );
        }

# Bug 96857 -  MySQL meta files (pid file, socket, ..) should not be placed in db directory
        unlink("/opt/zimbra/db/mysql.pid") if ( -e "/opt/zimbra/db/mysql.pid" );
        unlink("/opt/zimbra/db/mysql.sock")
          if ( -e "/opt/zimbra/db/mysql.sock" );
    }
    my $localxml = XMLin("/opt/zimbra/conf/localconfig.xml");
    my $lc_attr  = $localxml->{key}->{zimbra_class_database}->{value};
    if ( defined($lc_attr) && $lc_attr eq "com.zimbra.cs.db.MySQL" ) {
        main::setLocalConfig( "zimbra_class_database",
            "com.zimbra.cs.db.MariaDB" );
    }

    $lc_attr =
      $localxml->{key}->{short_term_all_effective_rights_cache_expiration}
      ->{value};
    if ( defined($lc_attr) && $lc_attr + 0 != 50000 ) {
        main::setLdapServerConfig(
            $hn,
    ...
    $lc_attr =
      $localxml->{key}->{short_term_all_effective_rights_cache_size}->{value};
    if ( defined($lc_attr) && $lc_attr + 0 != 128 ) {
        main::setLdapServerConfig( $hn,
            'zimbraShortTermAllEffectiveRightsCacheSize', "$lc_attr" );
    }
    ...
    if ( defined($lc_attr) && $lc_attr + 0 != 128 ) {
        main::setLdapServerConfig( $hn, 'zimbraShortTermGranteeCacheSize',
            "$lc_attr" );
    }

    $lc_attr =
      $localxml->{key}->{zimbra_mailbox_throttle_reap_interval}->{value};
...
sub upgrade870BETA2 {
    ...
    if ( main::isInstalled("zimbra-mta") ) {
        ...
        if ( -e ${antispam_mysql_mycnf} ) {
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-mysql_basedir --section=mysqld --key=basedir --set --value='/opt/zimbra/common' ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-error-log --section=mysqld_safe --key=err-log --unset ${antispam_mysql_mycnf}"
            );
            main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-error-log --section=mysqld_safe --key=log-error --set --value=/opt/zimbra/log/mysqld.log ${antispam_mysql_mycnf}"
            );
        }
        main::setLdapServerConfig( $hn, 'zimbraMtaCommandDirectory',
            "/opt/zimbra/common/sbin" );
        main::setLdapServerConfig( $hn, 'zimbraMtaDaemonDirectory',
            "/opt/zimbra/common/libexec" );
        main::setLdapServerConfig( $hn, 'zimbraMtaMailqPath',
            "/opt/zimbra/common/sbin/mailq" );
        main::setLdapServerConfig( $hn, 'zimbraMtaManpageDirectory',
            "/opt/zimbra/common/share/man" );
        main::setLdapServerConfig( $hn, 'zimbraMtaNewaliasesPath',
            "/opt/zimbra/common/sbin/newaliases" );
        main::setLdapServerConfig( $hn, 'zimbraMtaSendmailPath',
            "/opt/zimbra/common/sbin/sendmail" );

        # Bug 98771 - Add support for DANE
        my $dns_setting =
          main::getLdapServerValue("zimbraMtaDnsLookupsEnabled");
        if ( lc($dns_setting) eq "false" ) {
            main::setLdapServerConfig( $hn, 'zimbraMtaSmtpDnsSupportLevel',
                'disabled' );
        }
        ...
        main::runAsZimbra(
            "/opt/zimbra/bin/postconf -e sender_canonical_maps=''");
    }
    if ( main::isFoss() ) {
...
sub upgrade870RC1 {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        if ($isLdapMaster) {
            main::setLdapGlobalConfig( "zimbraSSLDHParam",
        ...
        if ( !@attrvals ) {
            $result = $ldap->modify( $dn, add => { olcDbRtxnSize => 0 }, );
        }
        $ldap->unbind;
    }
    return 0;
...
# behavior for all upgrades until the administrator decides to enable the strict name enforcement.
    ...
    upgradeLdapConfigValue( "zimbraImapLoadBalancingAlgorithm",
        "AccountIdHash", "ClientIpHash" );
    return 0;
}
...
sub isSqlRunning {
    my $rc = 0xffff &
      system("$su \"/opt/zimbra/bin/mysqladmin status > /dev/null 2>&1\"");
    $rc = $rc >> 8;
    return ( $rc ? undef : 1 );
}
...
sub isLoggerSqlRunning {
    my $rc = main::runAsZimbra(
        "/opt/zimbra/bin/logmysqladmin status > /dev/null 2>&1");
    return ( $rc ? undef : 1 );
}
...
sub runSchemaUpgrade {
    ...
    if ( !-x "${scriptDir}/$updateScripts{$curVersion}" ) {
        main::progress(
"Can't run ${scriptDir}/$updateScripts{$curVersion} - not executable!\n"
        );
        return 1;
    }
    ...
    open( MIG,
"$su \"/usr/bin/perl -I${scriptDir} ${scriptDir}/$updateScripts{$curVersion}\" 2>&1|"
    );
    while (<MIG>) {
        main::progress($_);
    }
...
sub cleanPostfixLC {
    ...
    my ( $var, $val );
    foreach $var (
        qw(command_directory daemon_directory mailq_path manpage_directory newaliases_path queue_directory sendmail_path)
      )
    {
...
sub updateMySQLcnf {
    ...
    if ( -e "$mycnf" ) {
        ...
        my $zimbra_user =
          qx(${zmlocalconfig} -m nokey zimbra_user 2> /dev/null) || "zimbra";
        my $zimbra_tmp_directory =
             qx(${zmlocalconfig} -m nokey zimbra_tmp_directory 2> /dev/null)
          || "/opt/zimbra/data/tmp";
        open( TMP, ">$tmpfile" );
...
sub doMysql55Upgrade {
    my $mysql_mycnf = main::getLocalConfig("mysql_mycnf");
    my $zimbra_log_directory =
      main::getLocalConfig("zimbra_log_directory") || "/opt/zimbra/log";
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion} --section=mysqld --unset --key=ignore-builtin-innodb ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion} --section=mysqld --unset --key=plugin-load ${mysql_mycnf}"
    );
}
...
sub doAntiSpamMysql55Upgrade {
    ...
    if ( -e ${antispam_mysql_mycnf} ) {
        main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion} --section=mysqld --unset --key=ignore-builtin-innodb ${antispam_mysql_mycnf}"
        );
        main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion} --section=mysqld --unset --key=plugin-load ${antispam_mysql_mycnf}"
        );
    }
}
...
sub doMysql56Upgrade {
    ...
    my $zimbra_log_directory =
      main::getLocalConfig("zimbra_log_directory") || "/opt/zimbra/log";
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-table_cache-fixup --section=mysqld --key=table_cache --unset ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-table_open_cache-fixup --section=mysqld --key=table_open_cache --setmin --value=1200 ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-innodb_data_file_path-fixup --section=mysqld --set --key=innodb_data_file_path --value=ibdata1:10M:autoextend ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-long-query-time-fixup --section=mysqld --unset --key=long-query-time ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-long_query_time-fixup --section=mysqld --set --key=long_query_time --value=1 ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-log-queries-not-using-indexes-fixup --section=mysqld --unset --key=log-queries-not-using-indexes ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-log_queries_not_using_indexes-fixup --section=mysqld --set --key=log_queries_not_using_indexes ${mysql_mycnf}"
    );
}
...
sub doMariaDB101Upgrade {
    ...
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-mysql_socket --section=mysqld --key=socket --set --value='/opt/zimbra/data/tmp/mysql/mysql.sock' ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-mysql_pidfile --section=mysqld --key=pid-file --set --value='/opt/zimbra/log/mysql.pid' ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-mysql_pidfile --section=mysqld_safe --key=pid-file --set --value='/opt/zimbra/log/mysql.pid' ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-mysql_basedir --section=mysqld --key=basedir --set --value='/opt/zimbra/common' ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-error-log --section=mysqld_safe --key=err-log --unset ${mysql_mycnf}"
    );
    main::runAsZimbra(
"/opt/zimbra/libexec/zminiutil --backup=.pre-${targetVersion}-error-log --section=mysqld_safe --key=log-error --set --value=/opt/zimbra/log/mysqld.log ${mysql_mycnf}"
    );
}
...
sub doMysqlUpgrade {
    ...
    my $cmd =
"$mysqlUpgrade --defaults-file=$mysql_mycnf -S $mysql_socket --user=root --password=$db_pass";
...
sub doBackupRestoreVersionUpdate($) {
    ...
    my (
        $prevRedologVersion, $currentRedologVersion,
        $prevBackupVersion,  $currentBackupVersion
    );
    $prevRedologVersion = &Migrate::getRedologVersion;
    $currentRedologVersion =
      qx($su "zmjava com.zimbra.cs.redolog.util.GetVersion");
    ...
    if ( $prevRedologVersion != $currentRedologVersion ) {
        ...
        Migrate::updateRedologVersion( $prevRedologVersion,
            $currentRedologVersion );
    ...
    if ( -f "/opt/zimbra/lib/ext/backup/zimbrabackup.jar" ) {
        ...
        $currentBackupVersion =
          qx($su "zmjava com.zimbra.cs.backup.util.GetVersion");
        ...
        if ( $prevBackupVersion != $currentBackupVersion ) {
            ...
            Migrate::updateBackupVersion( $prevBackupVersion,
                $currentBackupVersion );
            main::progress("Backup version update finished.\n");
        }
    }
    my ( $currentMajorBackupVersion, $currentMinorBackupVersion ) =
      split( /\./, $currentBackupVersion );
    my ( $prevMajorBackupVersion, $prevMinorBackupVersion ) =
      split( /\./, $prevBackupVersion );
    ...
    main::progress(
"Moving /opt/zimbra/backup/* to /opt/zimbra/backup/${startVersion}-${currentBackupVersion}.\n"
    );
    clearBackupDir( "/opt/zimbra/backup",
        "${startVersion}-${currentBackupVersion}" );
    main::progress(
"Moving /opt/zimbra/redolog/* to /opt/zimbra/redolog/${startVersion}-${currentRedologVersion}.\n"
    );
    clearRedologDir( "/opt/zimbra/redolog",
        "${startVersion}-${currentRedologVersion}" );
...
sub reloadLdap($) {
    my ($upgradeVersion) = @_;
    if ( main::isInstalled("zimbra-ldap") ) {
        if ( $main::migratedStatus{"LdapReloaded$upgradeVersion"} ne
            "CONFIGURED" )
        {
            my $ldifFile = "/opt/zimbra/data/ldap/ldap-accesslog.bak";
            if (
                -d '/opt/zimbra/data/ldap/config/cn=config/olcDatabase={3}mdb' )
            {
...
qx(mv /opt/zimbra/data/ldap/accesslog /opt/zimbra/data/ldap/accesslog.prev);
                        ...
                        my $rc;
                        $rc = main::runAsZimbra(
                            "/opt/zimbra/libexec/zmslapadd -a $ldifFile");
                        if ( $rc != 0 ) {
                            main::progress(
                                "slapadd import of accesslog db failed.\n");
                            return 1;
                        }
                        main::progress("done.\n");
                    }
...
qx(mv /opt/zimbra/data/ldap/mdb.prev /opt/zimbra/data/ldap/mdb.prev.$$);
                ...
                my $rc;
                $rc =
                  main::runAsZimbra("/opt/zimbra/libexec/zmslapadd $ldifFile");
                if ( $rc != 0 ) {
                ...
                if ( !-f $ldifFile ) {
                    main::progress(
                        "Error: Unable to find /opt/zimbra/data/ldap/ldap.bak\n"
                    );
                }
                else {
                    main::progress(
...
sub upgradeLdap($) {
    ...
    if ( main::isInstalled("zimbra-ldap") ) {
        if ( $upgradeVersion eq "8.7.0_BETA2" ) {
            ...
            {
                if (
                    -f '/opt/zimbra/data/ldap/config/cn=config/cn=module{0}.ldif'
                  )
                {
                    my $infile =
"/opt/zimbra/data/ldap/config/cn\=config/cn\=module\{0\}.ldif";
...
/^olcModulePath: \/opt\/zimbra\/openldap\/sbin\/openldap/
                        ...
                        {
                            print OUT
"olcModulePath: \/opt\/zimbra\/common\/libexec\/openldap\n";
                            next;
                        }
                        if ( $_ =~ /^# CRC32/ ) {
                            next;
            ...
            {
                unlink(
"/opt/zimbra/data/ldap/config/cn\=config/cn\=schema/cn\=\{7\}pgp-keyserver.ldif"
                );
                if ( -f '/opt/zimbra/data/ldap/config/cn=config.ldif' ) {
                    ...
                    while (<IN>) {
                        if ( $_ =~ /^olcPidFile: / ) {
                            print OUT
"olcPidFile: /opt/zimbra/data/ldap/state/run/slapd.pid\n";
                            next;
                        }
                        if ( $_ =~ /^olcArgsFile: / ) {
                            print OUT
"olcArgsFile: /opt/zimbra/data/ldap/state/run/slapd.args\n";
                            next;
                        }
                        if ( $_ =~ /^# CRC32/ ) {
                            next;
...
"olcModulePath: \/opt\/zimbra\/common\/libexec\/openldap\n";
                ...
                "CONFIGURED" )
            {
                ...
                unlink(
"/opt/zimbra/data/ldap/config/cn\=config/cn\=schema/cn\=\{3\}zimbra.ldif"
                );
                unlink(
"/opt/zimbra/data/ldap/config/cn\=config/cn\=schema/cn\=\{4\}amavisd.ldif"
                );
                unlink(
"/opt/zimbra/data/ldap/config/cn\=config/cn\=schema/cn\=\{7\}pgp-keyserver.ldif"
                );
                ...
                if ( -f $ldifFile && -s $ldifFile ) {
                    ...
                    while (<IN>) {
                        ...
                        if (
                            $_ =~ /^zimbraPrefStandardClientAccessilbityMode:/ )
                        {
                            next;
                        }
                        if ( $_ =~ /^objectClass: zimbraHsmGlobalConfig/ ) {
                            next;
                        }
...
"olcArgsFile: /opt/zimbra/data/ldap/state/run/slapd.args\n";
                    ...
                    if (
                        -f '/opt/zimbra/data/ldap/config/cn=config/olcDatabase={3}hdb.ldif'
                      )
                    {
qx(mv /opt/zimbra/data/ldap/config/cn\=config/olcDatabase\=\{3\}hdb.ldif /opt/zimbra/data/ldap/config/cn\=config/olcDatabase=\{3\}mdb.ldif);
                        $infile =
"/opt/zimbra/data/ldap/config/cn\=config/olcDatabase\=\{3\}mdb.ldif";
...
"olcDbDirectory: /opt/zimbra/data/ldap/mdb/db\n";
                    ...
                    if (
                        -f '/opt/zimbra/data/ldap/config/cn=config/olcDatabase={2}hdb.ldif'
                      )
                    {
qx(mv /opt/zimbra/data/ldap/config/cn\=config/olcDatabase\=\{2\}hdb.ldif /opt/zimbra/data/ldap/config/cn\=config/olcDatabase\=\{2\}mdb.ldif);
                        $infile =
"/opt/zimbra/data/ldap/config/cn\=config/olcDatabase\=\{2\}mdb.ldif";
...
qx(mv /opt/zimbra/data/ldap/mdb /opt/zimbra/data/ldap/mdb.prev);
                    ...
                    my $rc;
                    $rc = main::runAsZimbra(
                        "/opt/zimbra/libexec/zmslapadd $slapoutfile");
                    if ( $rc != 0 ) {
                        main::progress("slapadd import failed.\n");
                        return 1;
                    }
...
sub migrateLdap($) {
    my ($migrateVersion) = @_;
    if ( main::isInstalled("zimbra-ldap") ) {
        if ( $main::migratedStatus{"LdapUpgraded$migrateVersion"} ne
            "CONFIGURED" )
        {
            if ( -f "/opt/zimbra/data/ldap/ldap.bak" ) {
                ...
                if ( -s $infile ) {
                    ...
                    main::progress(
"LDAP backup file /opt/zimbra/data/ldap/ldap.bak is empty.\n"
                    );
                    main::progress(
                        "Valid LDAP backup file not found, exiting.\n");
                    return 1;
                }
...
qx(mv /opt/zimbra/data/ldap/mdb.prev /opt/zimbra/data/ldap/mdb.prev.$$);
                ...
                my $rc;
                $rc =
                  main::runAsZimbra("/opt/zimbra/libexec/zmslapadd $outfile");
                if ( $rc != 0 ) {
                    main::progress("slapadd import failed.\n");
                    return 1;
                }
...
sub migrateAmavisDB($) {
    ...
    foreach my $fromVersion (qw(2.5.2 2.4.3 2.4.1 2.3.3 2.3.1)) {
        ...
        if ( -d "$fromDir/db" && -d "$toDir" && !-e "$toDir/db/cache.db" ) {
            main::progress(
"Migrating amavis-new db from version $fromVersion to $toVersion\n"
            );
        ...
        if (   -d "$fromDir/.spamassassin/"
            && -d "$toDir"
            && !-e "$toDir/.spamassassin/bayes_toks" )
        {
            main::progress(
"Migrating amavis-new .spamassassin from version $fromVersion to $toVersion\n"
            );
...
sub relocateAmavisDB() {
    ...
    if (   -d "$fromDir/.spamassassin/"
        && -d "$toDir"
        && !-e "$toDir/.spamassassain/bayes_toks" )
    {
        qx(rm -rf $toDir/.spamassassin > /dev/null 2>&1);
        qx(mv $fromDir/.spamassassin $toDir/.spamassassin);
        qx(chown zimbra:zimbra $toDir/.spamassassin);
...
sub upgradeAllGlobalAdminAccounts {
    ...
    if ( my $pid = open3( $wfh, undef, $efh, "$su \"$ZMPROV\"" ) ) {
        foreach my $admin (@adminUpgrades) {
            main::detail(
                "$ZMPROV ma $admin zimbraAdminConsoleUIComponents cartBlancheUI"
            );
            print $wfh
              "ma $admin zimbraAdminConsoleUIComponents cartBlancheUI\n";
        }
        print $wfh "exit\n";
        @errors = <$efh>;
...
sub addLdapIndex($$$) {
    ...
    if ( !$hasIndex ) {
        $result =
          $ldap->modify( $dn, add => { olcDbIndex => "$index $type" }, );
    }
    $ldap->unbind;
    return !$hasIndex;
}
...
sub runAttributeUpgrade($) {
    my ($startVersion) = @_;
    my $rc = main::runAsZimbra(
"zmjava com.zimbra.cs.account.ldap.upgrade.LdapUpgrade -b 27075 -v $startVersion"
    );
    return $rc;
}
...
sub runLdapAttributeUpgrade($) {
    ...
    return if ( $bug eq "" );
    my $rc = main::runAsZimbra(
        "zmjava com.zimbra.cs.account.ldap.upgrade.LdapUpgrade -b $bug -v");
    return $rc;
}
